...
class ReadLine(CallableTool2[ParamsLine]):
    name: str = "ReadLine"
    params: type[ParamsLine] = ParamsLine

    def __init__(self, runtime: Runtime) -> None:
        description = (
            "Read specific lines from a file. "
            "Useful when you need to read a specific range of lines."
        )
    ...
    @override
    async def __call__(self, params: ParamsLine) -> ToolReturnValue:
        if not params.path:
            return ToolError(
        ...
        try:
            ...
            lines: list[str] = []
            current_line_no = 0
            end_line = params.start + params.count - 1

            async for line in p.read_lines(errors="replace"):
                current_line_no += 1
                if current_line_no < params.start:
                    continue
            ...
            for line_num, line in zip(
                range(params.start, params.start + len(lines)), lines, strict=True
            ):
                lines_with_no.append(f"{line_num:6d}\t{line}")

            message = f"{len(lines)} lines read from file starting from line {params.start}."
            if len(lines) == 0:
                message = "No lines read from file (start line beyond file length)."
            elif len(lines) < params.count:
