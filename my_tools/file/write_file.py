...
# Function map: file extension -> validator function
# Validator returns None if valid, error message string if invalid
EXTENSION_VALIDATORS: dict[str, Callable[[str], str | None]] = {
    ".json": check_json,
    ".xml": check_xml,
}
...
class WriteFile(CallableTool2[Params]):
    name: str = "WriteFile"
    description: str = 'Write content to file in path'
    ...
    async def __call__(self, params: Params) -> ToolReturnValue:
        ...
        try:
            ...
            extension = Path(str(p)).suffix.lower()
            if params.mode == 'overwrite':
                if validator := EXTENSION_VALIDATORS.get(extension):
                    if error_msg := validator(params.content):
                        return ToolError(
                            message=f"File was written but validation failed: {error_msg}",
