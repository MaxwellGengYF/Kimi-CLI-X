...
from my_tools.check_fmt import check_json_str, check_xml_str
...
class WriteFile(CallableTool2[Params]):
    ...
    async def __call__(self, params: Params) -> ToolReturnValue:
        ...
        try:
            ...
            # Validate JSON/XML content based on file extension
            ...
            if file_ext == ".json":
                if error_msg := check_json_str(new_text):
                    return ToolError(
                        message=f"Invalid JSON content: {error_msg}. But file successfully {action}. Current size: {file_size} bytes.",
                        brief="JSON validation failed",
                    )
            elif file_ext == ".xml":
                if error_msg := check_xml_str(new_text):
                    return ToolError(
                        message=f"Invalid XML content: {error_msg}, But file successfully {action}. Current size: {file_size} bytes.",
                        brief="XML validation failed",
                    )
