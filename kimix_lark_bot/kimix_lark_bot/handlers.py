...
class TaskHandler(BaseHandler):
    def handle(self, plan: ActionPlan, chat_id: str, message_id: str, ctx: ConversationContext) -> None:
        ...
        if not path:
            ...
            if not running:
                self.ctx.messaging.reply_card(message_id, error("未找到会话", "没有正在运行的 Kimix 会话。\n请先启动一个，例如：启动 myproject"))
                return
            if len(running) == 1:
                path = running[0].path
                ...
                self.ctx.messaging.reply_text(message_id, "有多个会话运行中：" + ", ".join(names) + "\n请指定工作区")
                return
        ...
        def do_task() -> None:
            ...
            def _poll_progress():
                ...
                while not stop_event.wait(3.0):
                    ...
                    card = progress(title="🚀 任务已提交", description=desc)
                    if prog_mid:
                        self.ctx.messaging.update_card(prog_mid, card)
            ...
            poll_thread.start()
            try:
                ok, proc, start_msg = self.ctx.process_mgr.ensure_running(path, chat_id)
            except Exception as exc:
                stop_event.set()
                self.ctx.op_tracker.finish(op_id)
                logger.error("[%s] Exception in do_task: %s", chat_id, exc, exc_info=True)
