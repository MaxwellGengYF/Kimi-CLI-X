...
| **记忆系统 (Remember / Recall)** | 支持跨会话持久化存储与检索。`Remember` 将事实、观察或知识按重要性分级存入长期/短期记忆；`Recall` 按语义查询从多层记忆（working、episodic、semantic）中召回相关内容，并可按标签过滤。 |
...
| [`docs/tutorials/6_memory.md`](docs/tutorials/6_memory.md) | **记忆系统教程**。介绍三层记忆架构（L1/L2/L3）与四款工具（Remember、Recall、GetContext、Reflect）的参数说明、调用流程及最佳实践。 |
