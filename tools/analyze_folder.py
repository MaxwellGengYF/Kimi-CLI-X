...
def main():
    ...
    README_PATH = "./analysis_output/analyze.md"
    ...
    Output a structured summary in JSON format with these fields to {OUTPUT_DIR}/structure.json:
    - project_type: string
    ...
    json_dir = Path(OUTPUT_DIR)/'structure.json'
    if not json_dir.exists():
        print(f"Error: {json_dir} does not exist. Please run structure analysis first.")
        return
    ...
    todo_prompt = f'''
    Based on the following project structure (JSON):
    {json_dir}
    ...
    if todos:
        for todo in todos:
            if isinstance(todo, dict):
                ...
                output_file = todo.get(
                    'output_file', f"{OUTPUT_DIR}/{todo.get('id', 'unknown')}.md")
    ...
    if not Path(README_PATH).exists():
        print(f"Error: {README_PATH} was not generated.")
        return
